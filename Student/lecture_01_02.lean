...
cmd shift enter to open the lean infoview
-/
...
Nat: natural number -- zero, successor of zero (1), and so on
  4 is s(s(s(0)))

each type also has a type. Bool is type Type 0. Type 0 is of Type 1, and so on. this is the type universe
-/
