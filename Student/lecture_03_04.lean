...
Parametric polymorphism: same function body but just different types that are handled
-/
...
/-!
comp takes 5 arguments and returns a fucntion
let α
-/
...
#reduce is_even_len "yo"

def comp_imp {α β γ : Type} : (β → γ) → (α → β) → (α → γ)
| g, f => fun a => g (f a)

def is_even_len' := comp_imp is_even String.length

def apply_to (α : Type) : (α -> α) → (α → α)
| f => fun a => f (f a)

#reduce (apply_to inc) 5

-- types are values, functions are objects
/-!
